...
function selectMoveSkill(nodeId) {
  if (game.mode !== Mode.LANTERN) {
    return;
  }
  ...
  game.activeMoveSkillId = game.activeMoveSkillId === nodeId ? null : nodeId;
  updateMoveSkillPanel(game.now);
}
...
function handleCanvasClick(event) {
  ...
  if (game.mode === Mode.LANTERN) {
    ...
    if (game.activeMoveSkillId !== null) {
      const moved = castMoveLanternSkill(game.activeMoveSkillId, point);
      if (moved) {
        game.activeMoveSkillId = null;
      }
      updateMoveSkillPanel(game.now);
...
window.addEventListener("keydown", (event) => {
  ...
  if (event.key === "1" || event.key === "2" || event.key === "3") {
    ...
    const moved = castMoveLanternSkill(nodeId, { x: mouseX, y: mouseY });
    if (moved) {
      game.activeMoveSkillId = null;
      updateMoveSkillPanel(game.now);
    }
